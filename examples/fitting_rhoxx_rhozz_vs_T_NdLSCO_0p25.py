...
## Function residual ########
def residualFunc(pars, bandObject, T_array, rhoxx_data, rhozz_data):
    ...
    diffxx = rhoxx_data - rhoxx_fit
    diffzz = rhozz_data - rhozz_fit

    return np.concatenate((diffxx, diffzz))
...
## Run fit algorithm
out = minimize(residualFunc, pars, args=(bandObject, T_array, rhoxx_data, rhozz_data))
...
rhozz_fit = np.empty_like(T_fit, dtype=np.float64)
...
print("hbar*B/kB = " + str(np.round(B*1e12*hbar/Boltzmann, 2)))
...
line = axes.plot(T_array, rhozz_data * 1e5, label=r"data")
plt.setp(line, ls ="-", c = '#ff0000', lw = 3, marker = "", mfc = '#ff0000', ms = 7, mec = '#ff0000', mew= 0)

line = axes.plot(T_fit, rhozz_fit * 1e5, label=r"fit")
plt.setp(line, ls ="-", c = '#000000', lw = 1, marker = "", mfc = '#000000', ms = 7, mec = '#000000', mew= 0)

#############################################
axes.set_xlim(0, np.max(T_array))
# axes.set_ylim(0, 1.25*np.max(rhozz_array*1e8))
axes.set_ylim(bottom=0)
axes.tick_params(axis='x', which='major', pad=7)
axes.tick_params(axis='y', which='major', pad=8)
axes.set_xlabel(r"$T$ ( K )", labelpad=8)
