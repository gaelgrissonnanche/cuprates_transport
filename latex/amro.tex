...
The remaining sections of this document will explain how to compute $\sigma_{ij}(\epsilon)$ in various limit of the semi-classical approximation. One should note, however, that the above formulation of transport coefficients is valid in the strongly correlated case, with $\sigma_{ij}(\epsilon)$ corresponding to the current-current correlation function.
First, however, let us define the usual measurable transport coefficients (resistivity, Hall coefficient, Seebeck coefficient, etc.) in 2D, and give an example of how one can use the above formulation to think about those coefficients in cuprates.
...
\subsection{Example: Interpretation of the Seebeck effect}
...
\end{align}
but it is also valid in the strongly correlated case, for which $\sigma_{xx}(\epsilon)$ would simply be the current-current correlation function. In the first approximation, the latter would be:
\begin{align}
\sigma_{xx}(\epsilon) = 
&=
e^2\iiint_{\text{BZ}}\frac{d^3k}{(2\pi)^3}
...
Finally, let us add that above correspondence for the longitudinal conductivity has an analogous one for the transverse conductivity (not shown here). Together, these suggest the following heuristic:
\begin{align}
...
\end{align}
to go from the non-interacting case to the weakly interacting case. The validity of this heuristic has not been verified thoroughly for all the versions of $\sigma_{xx}(\epsilon)$ derived in the following sections, but it may serve as a guide to derive their strongly correlated versions.
...
At this point, we can reduce all the tensors into a single expression. We substitute \eqref{chambers2} and \eqref{velocity} in \eqref{chambers1}, and then substitute the result in \eqref{sigma}, \eqref{alpha}, and \eqref{beta}, unifying them in the following expression:
\begin{align}
\sigma_{ij}^{(n)}
&=
\int_{-\infty}^{\infty}d\epsilon
...
The Dirac delta $\delta(\epsilon-E(\vec k))$ allow to change the three-dimensional $\vec k$ integral to a two-dimensional integral over the surface where the delta's argument is zero; the surface defined by $E(\vec k)=\epsilon$ (see equation~8.57 and 8.62 of Ashcroft~\&~Mermin~\cite{ashcroft_solid_1976} for a similar statement). Take an arbitrary function $F(\vec k)$, for example:
\begin{align}
\iiint d^3 k 
...
\end{empheq}
where $\delta_{ij}$ is the Kronecker symbol and $\varepsilon_{ijk}$ is the Levi-Cevita symbol (which is 0 if $i=j$). This is the case mentionned at the beginning, with the first term of this equation corresponding to~\eqref{eq_will_return}.
...
\end{align}
Unfortunately, this simplification does not extend to $\tensor{\alpha}$ and $\tensor{\beta}$ for which one must instead turn to Sommerfeld expansions (see Appendix~C of Ashcroft~\&~Mermin~\cite{ashcroft_solid_1976}).
