...
from cuprates_transport.Conductivity.scat_rate_models import *
##<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<#
...
class Conductivity:
    def __init__(self, bandObject, Bamp, Bphi=0, Btheta=0, N_time=500,
        ...
        # Time parameters
        # TODO: Is this really necessary? Can't it be computed on the fly? It forces to have a pre-run bandstructure
