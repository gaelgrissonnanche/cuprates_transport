import numpy as np
from numpy import pi, exp, arctan2
...
def figScatteringColor(condObj, kz=0, gamma_min=None, gamma_max=None, mesh_xy=501):
    bObj = condObj.bandObject
    ...
    for contour in contours:
        ...
        gamma_kz = 1 / condObj.tau_total_func(kx, ky, kz, vx, vy, vz)
...
def figScatteringPhi(condObj, kz=0, mesh_xy=501):
    bObj = condObj.bandObject
...
def figOnekft(condObj, index_kf = 0, meshXY = 1001):
    mesh_graph = meshXY
    bObj = condObj.bandObject
    ...
    line = axes.plot(condObj.kft[0, index_kf,:]*bObj.a, condObj.kft[1, index_kf,:]*bObj.b)
    ...
    line = axes.plot(condObj.kft[0, index_kf, -1]*bObj.a, condObj.kft[1, index_kf, -1]*bObj.b)
...
def figOnevft(condObj, index_kf = 0):
    ...
    line = axes.plot(condObj.time_array, condObj.vft[2, index_kf,:])
...
def figCumulativevft(condObj, index_kf = -1):
    fig, axes = plt.subplots(1, 1, figsize = (9.2, 5.6))
    fig.subplots_adjust(left = 0.17, right = 0.81, bottom = 0.18, top = 0.95)

    line = axes.plot(condObj.time_array, np.cumsum(condObj.vft[2, index_kf, :] * exp(-condObj.t_o_tau[index_kf, :])))
...
def figdfdE(condObj):
    ...
    dfdE_cut    = condObj._dfdE_cut_percent * np.abs(condObj.dfdE(0))
    epsilon_cut = condObj.energyCutOff(dfdE_cut)
    d_epsilon   = condObj.epsilon_array[1]-condObj.epsilon_array[0]
    epsilon = np.arange(-epsilon_cut-10*d_epsilon, epsilon_cut+11*d_epsilon, d_epsilon)
    dfdE = condObj.dfdE(epsilon)
...
def figParameters(condObj, fig_show=True):
    bObj = condObj.bandObject
    ...
    fig.text(0.72, 0.86, "Scattering Parameters",
                fontsize=16, color='#008080')
    label_parameters = [
        r"$\Gamma_{\rm 0}$       = " + "{0:.1f}".format(condObj.gamma_0) +
        "   THz",
        r"$\Gamma_{\rm DOS}^{\rm max}$   = " +
        "{0:.1f}".format(condObj.gamma_dos_max) + "   THz",
        r"$\Gamma_{\rm k}$       = " + "{0:.1f}".format(condObj.gamma_k) +
        "   THz",
        r"$n$         = " + "{0:.1f}".format(condObj.power),
        r"$A_{\rm arcs}$   = " + "{0:.1f}".format(condObj.factor_arcs),
        r"$\Gamma_{\rm tot}^{\rm max}$    = " +
        "{0:.1f}".format(condObj.gamma_tot_max) + "   THz",
        r"$\Gamma_{\rm tot}^{\rm min}$     = " +
        "{0:.1f}".format(condObj.gamma_tot_min) + "   THz",
    ]
    h_label = 0.82
    for label in label_parameters:
    ...
    for contour in contours:
        ...
        gamma_kz0 = 1 / condObj.tau_total_func(kx, ky, 0, vx, vy, vz)
