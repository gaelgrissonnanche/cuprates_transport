...
class BandStructure:
    ...
    def __init__(self,
                 ...
                 res_xy=20, res_z=1,
                 parallel = True, march_square=False, cif_file = "",
                 **trash):
        """
    ...
    def mc_func(self):
        ...
        try:
            ...
            exit("ERROR: self.dks contains NaN. Are you using " \
                 "marching square?")
        vf = self.vf * meV * Angstrom
        # in Joule.m (because in the code vf is not divided by hbar)
        vf_perp = sqrt(vf[0, :]**2 + vf[1, :]**2)  # vf perp to B, in Joule.m
        prefactor = (hbar)**2 / (2 * pi) / self.res_z
        # divide by the number of kz to average over all kz
